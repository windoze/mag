...
### [DONE] H-1 归档 service 计划并为 ACP interface 起草新 PLAN.md + TODO.md
...
- **归档**：本文件（含全部 `[DONE]` 记录）与旧 `PLAN.md`（mag-service 主干计划）一并 `git mv` 进
  `docs/archive/2026-07-19-mag-service/`，并新增该目录下 `README.md`（一句话说明 + 链接回 `docs/DESIGN.md` /
  `docs/ACP.md`）。仓库根 `PLAN.md` / `TODO.md` 已被 ACP interface 的新内容替换。
- **新 `PLAN.md`（ACP interface）**：唯一设计输入指向 `docs/ACP.md`（+ `docs/DESIGN.md` §5/§3.0）；范围限定
  **mag-acp crate**、列非目标、现有代码锚点（acp crate v1.2.0 API + `MagService` 方法 + schema 类型出处）、
  M1–M5 里程碑、关键约束、离线测试策略、五步验证序列齐备。
- **新 `TODO.md`（ACP interface）**：顶部通用执行规则块；任务按实现顺序编号 `M<里程碑>-<序号>`，全部标题带
  `[TODO]`；每任务三段式（上下文 / 做什么 / 验证条件）+ 精确锚点（`docs/ACP.md` 节号、acp crate 类型/方法、
  `MagService` 方法）+ 完整验证序列（聚焦测试过滤名 + 五步）；每个 milestone 末尾有 `M<n>-R` review 任务。
- **锚点核对**：acp crate = `agent-client-protocol` v1.2.0、schema = `agent-client-protocol-schema` v1.4.0
  （经 `../agent-lib/Cargo.lock` 确认真实存在）；`MagService` / `ServiceEvent` / wire 类型均对照
  `crates/mag-service/src/{lib.rs,service.rs}` 实际定义，无悬空引用。
- **验证**：纯文档任务，无需 `cargo`；`git diff --check` 干净。
