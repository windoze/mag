...
cancel）。GUI/web（I4）要行使的是**近全集**：多会话管理、`Question`/`Choice` 交互、delegation
树、pivot、source 运维面、配置读写。直接上 GUI 的风险（`DESIGN.md` §11 risk 6）是这些方法
...
- **external agent 管线的最早试炼场**：external ACP agent 委派是本程序的核心功能，提前落地、
  提前暴露问题（见决策 D3）。
...
| 3 | 多 agent 编排 | supervisor 经 `ask_<name>` 委派（model-routed），被委派方**至少涵盖 external ACP agent**；`DelegationStarted/Finished/Failed/Message` 事件渲染 |
| 4 | agent 间协作 | 委派链路中的审批各层受控：子 agent（含 external ACP agent）的 `InteractionRequested` 穿透到 root 会话并可应答 |
...
- **不实现 dispatcher-routed 委派**（`DESIGN.md` §8.2）：第一版 multi-agent 用 model-routed
  （每 delegate 一个 `ask_<name>` 工具）。
...
### 3.3 子 agent 交互统一 pop 到 root 会话（决策 D5）

委派链中子 agent（LLM subagent / external ACP agent）的审批与提问**不经独立通道**，统一作为
root 会话的 `InteractionRequested` 发出，由 root 会话的界面（CLI 的单一交互队列、未来的 GUI
模态框）处理：

- mag-core 侧：子 agent 的审批 gate 与 root 会话共用同一 `IpcApproval` 实例（委派装配时注入；
  **依赖 agent-lib A1**——当前子 agent 的交互根本到不了父级 handler，见 §5A），
  并在事件上标注**来源**。`ServiceEvent::InteractionRequested` 向后兼容新增字段
  `#[serde(default)] origin: Option<InteractionOrigin>`（`InteractionOrigin { agent: AgentIdWire,
  delegate: Option<String>, depth: u32 }` 之类）；`None` = root agent 本人。子 agent 的
  Permission 类交互本就有 `actor` 字段，origin 与其互补（actor 是权限语义主体，origin 是
  渲染归属）。
- CLI 侧：pending 交互单一队列（§1.2），提示行格式如
  `[approval · codex(sub, depth 1)] shell: git push …`；应答仍是对 root 会话调
  `respond_interaction`，由 mag-core 路由回正确的 pending oneshot。
- 其它会话（非当前 root 的委派链）到达的交互：CLI 只打印「会话 X 有待应答交互」提示，
  `/resume` 切过去后处理——不引入「后台应答」复杂度。
...
| P7 | delegation 接线：model-routed `ask_<name>`，被委派方含 **external ACP agent** | mag-core / mag-sources | 决策 D3；`Delegation*` 事件已有变体、尚无人发；依赖 agent-lib A1/A3（§5A） |

> P2 的可行性依赖 agent-lib A1（子 agent 交互路由）；P3 的 `apply_config` 依赖 agent-lib A2
> （facade reconfigure）。见下节。
...
- **配置系统**：TOML 解析/行级错误；DTO↔DO round-trip 不丢字段；resolve 拓扑与悬空引用报错；
  ...
  （§4.4 四行各一个测试）；`apply_config` 经 turn-complete listener 在边界生效。
- **pivot**：边界内注入成功 / 窗口外重试后成功 / run 结束前未落地发 `PivotDropped` /
  无 run 时 `NotPivotable` / cancel 后队列清空。
- **external ACP agent 委派**：用 agent-lib 的 ACP client 方向经内存管道驱动一个 fake ACP
  server（或第二个 mag 实例的 `mag --acp`，缺环境 `#[ignore]`），断言 delegation trace、
  子 agent 审批穿透到 root、cancel 沿委派链传播。
...
| D3 | **multi-agent 至少涵盖 external ACP agent** | external agent 是本程序核心功能，委派接线（P7）把 `external-acp` 方向纳入本里程碑，尽早暴露问题；Claude Code/Codex/OpenCode 特化 adapter 推迟 |
...
| D5 | **子 agent 交互统一 pop 到 root 会话** | 委派链所有 `InteractionRequested` 经 root 会话发出、带 `origin` 来源标注，由 root 会话的单一交互队列处理——与 GUI/web 的模态队列同构（§3.3） |
